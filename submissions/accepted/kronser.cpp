...
int main(){
    ...
    // Precompute the total standard deviation of all subsets
    vector<vector<double> > std_dev(n, vector<double>(n, 0));
    for(int i = 0; i < n; i++){
        for(int j = i; j < n; j++){
            double avg = 0;
            for(int k = i; k <= j; k++){
            ...
            avg /= (j - i + 1);
            for(int k = i; k <= j; k++){
                std_dev[i][j] += pow(pop[k] - avg, 2);
            }
            std_dev[i][j] = sqrt(std_dev[i][j])/(j - i + 1);
        }
    }
    ...
    for(int k = 1; k <= c; k++){
        for(int j = 0; j < n; j++){
            for(int i = 0; i < j; i++){
                d[j][k] = min(d[j][k], d[i][k-1] + std_dev[i+1][j]);
                if(d[j][k] == d[i][k-1] + std_dev[i+1][j])
                    src[j][k] = i;
            }
        }
    }
